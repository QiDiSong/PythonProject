...
customer_input_shape = [533310, 3, 17]      # [n, c, feature_length]
sample_num = 533310
in_channels = 3         # DO GO PEAK
feature_length = 17     # 17 year for time series, feature length
kernel_size = 5
...
class DCAE(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=in_channels, out_channels=32, kernel_size=kernel_size, stride=1),
            nn.ReLU(),
            nn.Conv1d(in_channels=32, out_channels=64, kernel_size=kernel_size, stride=1),
            nn.ReLU(),
            nn.Conv1d(in_channels=64, out_channels=128, kernel_size=kernel_size, stride=1),
            nn.ReLU(),
            nn.Conv1d(in_channels=128, out_channels=128, kernel_size=kernel_size, stride=1),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(128 * (feature_length - (kernel_size - 1) * 4), 384),
            nn.ReLU(),
            nn.Linear(384, 5),
            nn.ReLU()
        )
        ...
        self.decoder = nn.Sequential(
            nn.Linear(5, 384),
            nn.ReLU(),
            nn.Linear(384, 128 * (feature_length - (kernel_size - 1) * 4)),
            nn.ReLU(),
            nn.Unflatten(1, (128, feature_length - (kernel_size - 1) * 4)),
            nn.ConvTranspose1d(in_channels=128, out_channels=128, kernel_size=kernel_size, stride=1),
            nn.ReLU(),
            nn.ConvTranspose1d(in_channels=128, out_channels=64, kernel_size=kernel_size, stride=1),
            nn.ReLU(),
            nn.ConvTranspose1d(in_channels=64, out_channels=32, kernel_size=kernel_size, stride=1),
            nn.ReLU(),
            nn.ConvTranspose1d(in_channels=32, out_channels=in_channels, kernel_size=kernel_size, stride=1),
            # nn.ReLU()
        )
...
# 创建一个示例输入张量
# example_input = torch.randn(sample_num, in_channels, feature_length)
...
num_entries_to_add = 534000 - processed_dataset.shape[0]  # 计算差额
padded_dataset = np.pad(processed_dataset, ((0, num_entries_to_add), (0, 0), (0, 0)), mode='constant', constant_values=0)
padded_dataset = padded_dataset.reshape(-1, 100, 3, 17)
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for batch, data in enumerate(trainloader):
            ...
            inputs = data
            inputs = inputs.to(device)
            inputs = inputs.view(-1, 3, 17)  # 调整输入以匹配模型的期望形状
            inputs = inputs.float()         # dataset is torch.int16, but model bias dtype is float32
            optimizer.zero_grad()
            # 前向传播
